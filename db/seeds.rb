...
8.times do |i|
  Player.create(name: "Player #{i}", email: "lucio#{i}@correo.com", password: '12345', is_admin: false)
end

8.times do |i|
  m1 = Match.find(1)
  f = Forecast.new(match: m1, player: Player.find(i + 1), home_goals: 1, away_goals: 2, result: 'AWAY',
                   tournament_id: 1)
  f.save
end
